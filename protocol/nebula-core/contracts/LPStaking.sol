...
contract LPStaking is Ownable, ReentrancyGuard {
    ...
    function getLockedNFTInfo(address _staker) public view returns (address _nftCollectionAddress, bool isNFTStaked, uint256 stakedNFTId) {
            _nftCollectionAddress = nftAddress[_staker]; 
            isNFTStaked = stakers[_staker].nftStaked; 

            stakedNFTId = type(uint256).max; // Indicates no NFT is staked

        if (isNFTStaked && stakedNFTs[_staker].length() > 0) {
            stakedNFTId = stakedNFTs[_staker].at(0);
        }
    }
